...
+
*--check-cv25519-bits*:::
Check whether low/high bits of Curve25519 ECDH subkey are correctly set.
RNP internally sets those bits to required values (low 3 bits and high bit must be zero) during decryption,
...
*--fix-cv25519-bits*:::
Set low/high bits of Curve25519 ECDH subkey to the correct values, and save a key.
